 ...
 * 代理Mapping拦截器
 */
public class DispatchProxyIpt implements Interceptor {

    private static final String METHOD_GET = "GET";
    private static final String METHOD_POST = "POST";


    private static DispatchProxyIpt dispatch;


    public static DispatchProxyIpt getInstance() {
        if (dispatch == null) {
            synchronized (DispatchProxyIpt.class) {
                if (dispatch == null) {
                    dispatch = new DispatchProxyIpt();
                }
            }
        }
        return dispatch;
    ...
    public Response intercept(Chain chain) throws IOException {
        ...
        for (String mappingUrl : map.keySet()) {
            ...
            if (method.isAnnotationPresent(GetMapping.class)) {
                ...
                response = handleResponse(chain, method, mappingUrl, methodMapping);
            }
    ...
    private Response handleResponse(
        ...
        if (flag) {
            ...
            System.out.println("该请求由本地代理服务处理: " + requestUrl);
