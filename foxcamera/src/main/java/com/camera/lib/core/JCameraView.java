...
import com.camera.lib.listener.CaptureListener;
import com.camera.lib.listener.ClickListener;
import com.camera.lib.listener.ErrorListener;
import com.camera.lib.listener.JCameraListener;
import com.camera.lib.listener.TypeListener;
import com.camera.lib.state.CameraMachine;
import com.camera.lib.util.BitmapUtil;
import com.camera.lib.util.DeviceGesture;
import com.camera.lib.util.FileUtil;
import com.camera.lib.util.LogUtil;
import com.camera.lib.util.ScreenUtil;
import com.camera.lib.view.CameraView;
...
public class JCameraView extends FrameLayout implements CameraInterface.CameraOpenOverCallback,
    ...
    private void initView() {
        ...
        this.waterMarkView = new WaterMarkView(getContext());
        this.addView(waterMarkView);
    ...
    public ImageView getLeftView() {
        return mCaptureLayout.getIvLeft();
    }

    public void reSetLeftView() {
        mCaptureLayout.resetIvLeft();
    }

    public void reSetRightView() {
        mCaptureLayout.resetIvRight();
    }

    public ImageView getRightView() {
        return mCaptureLayout.getIvRight();
    }
    ...
    public void onResume() {
        ...
        CameraInterface.getInstance().setToolView(mCaptureLayout.getIvLeft(), mCaptureLayout.getIvRight());
        CameraInterface.getInstance().setIRotate(this);
...
//                    Log.i("CJT", "result = " + (result - firstTouchLength));
    ...
    public JCameraView setJCameraLisenter(JCameraListener jCameraLisenter) {
    ...
    public void resetState(int type) {
        switch (type) {
            ...
            case TYPE_PICTURE:
                ...
                waterMarkView.setVisibility(VISIBLE);
                break;
            case TYPE_SHORT:
                break;
            case TYPE_DEFAULT:
    ...
    public void showPicture(Bitmap bitmap, boolean isVertical) {
        ...
        if (!isNormal) {
            Map pos = getWaterMarkView().getMarkPos();
            LinkedHashMap waterMark = getWaterMarkView().getWaterMark();
            bitmap = BitmapUtil.bmpMark(bitmap, waterMark, pos);
        }

        captureBitmap = bitmap;
        mPhoto.setImageBitmap(bitmap);
        mPhoto.setVisibility(VISIBLE);
        waterMarkView.setVisibility(INVISIBLE);
    ...
    public JCameraView isNormal(boolean normal) {
        this.isNormal = normal;
        waterMarkView.noWaterMark(normal);
        return this;
    }
}
