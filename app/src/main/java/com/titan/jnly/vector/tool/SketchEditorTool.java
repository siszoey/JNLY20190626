...
public class SketchEditorTool {

    Context context;

    private SketchEditorTool(Context context) {
     ...
     * 线转面 添加面 进行拓扑相交检查
     */
    private void addPolygon(final FeatureTable table, final Polygon geometry, Map<String, Object> map, String xbh) {
        try {
            if (map == null) {
    ...
    private void getnewFeature(final FeatureTable table, Map<String, Object> map, Geometry geometry, ArrayList<Feature> features, String xbbh) {
        ...
        if (geometry.getGeometryType() == GeometryType.POLYLINE) {
            ...
            ToastUtil.showLong(context, "数据类型错误");
            return;
        }
